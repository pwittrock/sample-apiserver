	...
	for _, o := range b.context.Order {
		if IsAPIResource(o) {
			...
			if apis := filepath.Dir(unversioned); apis != b.APIsPkg && len(b.APIsPkg) > 0 {
				panic(errors.Errorf(
					"Found multiple apis directory paths: %v and %v", b.APIsPkg, apis))
			} else {
				b.APIsPkg = apis
			}
		}
...
func (apigroup *APIGroup) DoType(t *types.Type) (*Struct, []*types.Type) {
	...
	for _, member := range t.Members {
		...
		if memberKind == "TypeMeta" || memberKind == "ObjectMeta" {
			memberGroup = "metav1"
		}
		if member.Embedded {
			memberName = ""
		...
		default:
			s.Fields = append(s.Fields, &Field{
				Type: memberGroup + ". " + memberKind,
				Name: memberName,
			})
		case member.Type.IsPrimitive():
			fallthrough
		case memberGroup == GetGroup(t):
			s.Fields = append(s.Fields, &Field{Type: memberKind, Name: memberName})
		}
